...
class Report {
    ...
    _normalizeProcessCov(v8ProcessCov, fileIndex) {
        const result = []
        for (const v8ScriptCov of v8ProcessCov.result) {
            if(v8ScriptCov.url && v8ScriptCov.url.includes('.js')) {
                const splitUrl = v8ScriptCov.url.split('/');
                v8ScriptCov.url = `${__dirname}/../build/static/js/${splitUrl[splitUrl.length - 1]}`;
                
                if (fs.existsSync(v8ScriptCov.url) ) {
